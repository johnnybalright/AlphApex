...
from configs.config import OUTPUT_DIR, OUTPUT_DIRx
...
def parcel_geometry(projectnumber, parcelid):
    """
    ...
    """
    try:
        setproctitle.setproctitle('alpha_parcel_geometry')
        ...
        parcel_id = parcelid
        in_gdf = gpd.read_file(IN_SHP_MAIN_SUBSET)
        ...
        gdf = gpd.GeoDataFrame(geometry=[out_geom])
        # result = parcel_geom(filename, parcel_id)
        # logger.debug(result)
        # try:
        #     if result:
        #         if hasattr(result[0], 'geometry'):
        #             if not result[0].geometry.empty:
        #                 in_gdf_shp = result[0].geometry.iloc[0]
        #             else:
        #                 logger.debug("Geometry attribute is empty.")
        #         else:
        #             logger.debug("No geometry attribute.")
        #     else:
        #         logger.debug("Result is empty.")
        #         river_frontage_length = 0
        #         gs = None
        #         gs_setback = None
        #         result = (gs, river_frontage_length, gs_setback)
        #         logger.debug("river_frontage_length: 0 FT due to exception/unknownerror")
        #         logger.debug("parcel_geometry:complete")
        #         return result
        # except Exception as e:
        #     logger.debug(f"parcel_geom:failed-{e}")
        #     river_frontage_length = 0
        #     gs = None
        #     gs_setback = None
        #     result = (gs, river_frontage_length, gs_setback)
        ...
        frontage_line_path = os.path.join(OUTPUT_DIR, "frontage_line.shp")
        gs_frontage.to_file(frontage_line_path)
        ...
        if len(unique_elements) == 2:
            ...
            logger.debug(oppo_line)
        else:
            logger.debug("There are no two exactly unique points to form a line.")
        ...
        setback75 = os.path.join(OUTPUT_DIR, "setback_line.shp")
        gs_setback75.to_file(setback75)
        gs_scale = gs.scale(8, 8)
        out_shp_scaled = os.path.join(OUTPUT_DIR, "parcel_boundary_scaled.shp")
        ...
        subprocess.run(
            [
                f"gdal_contour -a ELEV -3d -b 1 -i 1 -q \
...
#         parcel_geometry(projectnumber, parcelid)
