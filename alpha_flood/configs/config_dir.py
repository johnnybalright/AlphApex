...
from .config import PARENT_DIR, CONFIG_DIR
...
def generate_config_file(configs,
    ...
    for config in configs:
        ...
        if os.path.isdir(config['src']):
            ...
            logger.debug(f"Warning: Source path does not exist: {config['src']}")
...
def copy_template(src, dest_dir):
    ...
    Output:
    logger.debugs out a message indicating whether the file or directory has
    been copied successfully.
    """
    if os.path.isdir(src):
        shutil.copytree(src, os.path.join(dest_dir, os.path.basename(src)), dirs_exist_ok=True)
        logger.debug(f"Directory {src} copied to {dest_dir}")
    elif os.path.isfile(src):
        if not os.path.exists(dest_dir):
            os.makedirs(dest_dir)
        shutil.copy(src, dest_dir)
        logger.debug(f"File {src} copied to {dest_dir}")
    else:
        logger.debug(f"No such file or directory: {src}")
...
def copy_templates_from_config(config_file):
    ...
    and logger.debugs out messages indicating the status of the copying
    process.
    """
